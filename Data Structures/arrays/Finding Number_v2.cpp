...
int rightbinarysearcy(int arr[], int l, int r, int x)
{
	if (r >= l)
	{
		int mid = l + (r - l) / 2;
		...
		// it can only be present in left subarray
		if (arr[mid] < x)
			return rightbinarysearcy(arr, l, mid - 1, x);

		// Else the element can only be present
		// in right subarray
		return rightbinarysearcy(arr, mid + 1, r, x);
	}
...
int printIndex(int arr[], int n, int key)
{
	...
	if (key == arr[max])
		return max;
	if (key < arr[max]) {
		index = binarySearch(arr, 0, max, key);
	}
	if(index == -1)
	{
		index = rightbinarysearcy(arr, max+1, n - 1, key);
	}
	return index;
}
int main()
